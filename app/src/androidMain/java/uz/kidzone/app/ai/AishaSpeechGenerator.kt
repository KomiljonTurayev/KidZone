...
object AishaSpeechGenerator {
    ...
    // Yaxshilangan: Tarmoq so'rovi IO thread'da bajariladi (Main Threadni qotirmaydi)
    private suspend fun post(text: String): Pair<Int, String> = withContext(Dispatchers.IO) {
        ...
        val body = buildString {
            ...
            // 1000-char cap matches the API's authenticated-request limit
    ...
    // Yaxshilangan: IO thread va Coroutine Delay
    private suspend fun pollForResult(id: Int): String? = withContext(Dispatchers.IO) {
        repeat(15) {
            delay(1_000)
            ...
            if (conn.responseCode in 200..299) {
                ...
                when (json.optString("status")) {
                    "SUCCESS" -> return@withContext json.getString("audio_path")
    ...
    // Yaxshilangan: Fayl yuklash jarayoni IO thread'ga o'tkazildi
    private suspend fun downloadAudio(audioPath: String, cacheDir: File): File? = withContext(Dispatchers.IO) {
